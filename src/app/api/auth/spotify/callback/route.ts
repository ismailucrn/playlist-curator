...
export async function GET(request: NextRequest) {
  try {
    if (!spotifyConfigured)
      throw new AppError("VALIDATION_ERROR", "Spotify yapılandırılmamış.");
    const spotifyError = request.nextUrl.searchParams.get("error");
    if (spotifyError)
      throw new AppError("UNAUTHORIZED", "Spotify erişim izni verilmedi.");

    const code = request.nextUrl.searchParams.get("code");
    const state = request.nextUrl.searchParams.get("state");
    const sealed = request.cookies.get(SPOTIFY_OAUTH_COOKIE)?.value;
    if (!code || !state || !sealed)
      throw new AppError("UNAUTHORIZED", "OAuth yanıtı eksik.");

    const attempt = unsealTransient<OAuthAttemptCookie>(sealed);
    if (attempt.state !== state || attempt.expiresAt < Date.now()) {
      throw new AppError(
        "UNAUTHORIZED",
        "OAuth state doğrulaması başarısız veya süresi dolmuş.",
      );
    }

    const tokens = await exchangeAuthorizationCode(code, attempt.verifier);
    if (!tokens.refresh_token)
      throw new AppError("UNAUTHORIZED", "Spotify refresh token döndürmedi.");
    ...
    if (!profileResponse.ok)
      throw new AppError("UNAUTHORIZED", "Spotify profili alınamadı.");
    ...
    const response = NextResponse.redirect(
      new URL("/dashboard?connected=1", env.APP_URL),
      303,
    );
    ...
    response.cookies.set(SPOTIFY_OAUTH_COOKIE, "", {
      httpOnly: true,
      maxAge: 0,
